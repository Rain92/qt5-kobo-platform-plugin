...
// On the upside, it appears to completely disable overlap checks and/or blending with the existing working
// buffer, which might come in handy when dealing with rotation...
...
// And now, massage the insanity that is the disp's character device ioctl handler into some sort of actually
// usable API... The rather loose typing and lack of proper IOC macros usage make the whole thing even more
// "interesting"...
...
typedef struct
{
    ...
    // ubuffer[1] (used alternatively as a size_t for the element count of the layer config copy, and an
    // unsigned int for the actual eink_update call. Must be > 0 and <= 16).
    ...
    unsigned long int
        cfa_use;  // ubuffer[6] (0, 1; cast to unsigned int, because bools are for losers, I guess?)
...
typedef struct
{
    ...
    // ubuffer[4], *outarg*, set on success (update_order in the eink buffer/pipeline manager; unrelated to
    // the layer_info's id).
...
typedef struct
{
    unsigned long int temp;  // ubuffer[0] (Cast to unsigned int, while the get variant returns a value stored
                             // in an int32_t...).
...
typedef struct
{
    ...
    struct disp_layer_config2*
        lyr_cfg2;                 // ubuffer[1], set channel & layer_id to identify the layer to return
...
// Shove everything into an union to ensure we never feed garbage to the ioctl handler for commands that
// actually read *less* than the prologue...
typedef union
{
